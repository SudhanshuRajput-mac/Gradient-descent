...
def create_step_by_step_animation(function_choice, start_x, learning_rate, num_steps):
    ...
    for step in range(num_steps + 1):
        ...
        steps_data.append({
            ...
            'next_x': next_x,
            'next_y': next_y,
            'learning_rate': learning_rate  # Store learning_rate in step data
        })
...
def main():
        ...
        if st.button("Play Step-by-Step Animation"):
            ...
            for step in range(num_steps + 1):
                with animation_placeholder.container():
                    ...
                    # Add a small delay between steps (except for the last step)
                    if step < num_steps:
                        time.sleep(2)
    ...
    with tab4:
        ...
        f_func = get_function(function_choice)
        gradient = calculate_gradient(function_choice, current_x)
        ...
        st.metric("Cost Improvement", f"{f_func(current_x) - f_func(new_x):.4f}")
